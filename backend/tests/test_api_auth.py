...
class TestAnalyticsConsent:
    """Tests for the analytics consent endpoint."""

    @staticmethod
    def _login(client: TestClient, existing_user: User) -> str:
        login_response = client.post(
            "/auth/login",
            json={
                "email": existing_user.email,
    ...
    def test_patch_analytics_consent_persists_accepted(
        self, client: TestClient, existing_user: User, session: Session
    ):
        """Accepting consent should persist 'accepted' on the user row."""
        token = self._login(client, existing_user)

        response = client.patch(
            "/users/me/analytics-consent",
            json={"consent": "accepted"},
            headers={"Authorization": f"Bearer {token}"},
        )

        assert response.status_code == 200
        session.refresh(existing_user)
        assert existing_user.analytics_consent == "accepted"

    def test_patch_analytics_consent_persists_declined(
        self, client: TestClient, existing_user: User, session: Session
    ):
        """Declining consent should persist 'declined' on the user row."""
        token = self._login(client, existing_user)
        ...
        assert response.status_code == 200
        session.refresh(existing_user)
        assert existing_user.analytics_consent == "declined"

    def test_patch_analytics_consent_requires_auth(self, client: TestClient):
    ...
    def test_patch_analytics_consent_rejects_invalid_value(
        self, client: TestClient, existing_user: User
    ):
        """Any value outside accepted/declined should be rejected."""
        token = self._login(client, existing_user)
