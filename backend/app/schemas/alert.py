...
class AlertRuleCreate(BaseModel):
    ...
    backtest_run_id: Optional[UUID] = None  # required for performance alerts; version is derived
    strategy_id: Optional[UUID] = None  # deprecated: ignored when backtest_run_id is provided
    ...
    def validate_by_type(self):
        ...
        if self.alert_type == AlertType.PERFORMANCE:
            # Performance alerts require backtest_run_id (version pinning)
            if not self.backtest_run_id:
                raise ValueError("backtest_run_id required for performance alerts")
            ...
            if self.backtest_run_id or self.strategy_id or self.threshold_pct is not None or self.alert_on_entry or self.alert_on_exit:
...
class AlertRuleResponse(BaseModel):
    ...
    last_fired_candle_ts: Optional[datetime]
