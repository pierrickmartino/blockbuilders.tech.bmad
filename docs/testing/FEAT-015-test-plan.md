...
- [x] Given a completed run with equity curve data, a line chart is displayed on the Backtest Results page
- [x] Chart plots equity (Y-axis) over time (X-axis) correctly
- [x] X-axis shows a reasonable number of auto-spaced time ticks (not overcrowded)
- [x] Y-axis shows equity values with currency-like formatting (no currency symbol required)
- [x] Chart uses Recharts line chart as specified
- [ ] Chart section has the title "Equity curve"

## 2. Tooltip Interaction

- [x] Hovering over a point on the chart shows a tooltip
- [x] Tooltip displays the timestamp formatted to user locale (or ISO short)
- [x] Tooltip displays the equity value rounded to 2 decimal places
- [x] Tooltip disappears when cursor moves away from the chart
...
- [x] Details are only fetched after run status is "completed"

## 4. Error & Empty States

- [x] When run status is Failed, the chart is NOT displayed
- [x] When run status is Failed, an error message from the run is shown instead of the chart
- [x] When equity curve data is missing for a completed run, "No equity data available for this run" message is displayed
- [ ] When equity data is missing, a "Retry loading" button is shown
- [x] Clicking "Retry loading" re-fetches the equity curve data
...
- [x] Timestamps in the response are ISO-8601 with UTC
- [ ] Details request is cached client-side to avoid refetch loops

## 6. Responsiveness

- [x] Chart stretches to fill container width on desktop
- [x] Chart stretches to fill container width on tablet
- [x] Chart stretches to fill container width on mobile
- [x] Chart does not overflow horizontally on any viewport
...
- [x] No unnecessary refetch of details data when the page re-renders

## 8. Timezone Handling

- [ ] Timestamps are stored and returned as UTC
- [x] Timestamps display consistently in the chart (UTC in data, formatted in UI)
- [ ] No timezone-related rendering issues (shifted data points, incorrect labels)

## 9. Page Layout Integration

- [x] Equity curve section appears in the correct position: after metrics summary, before trades table
