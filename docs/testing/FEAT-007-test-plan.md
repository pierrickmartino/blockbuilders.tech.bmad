...
- [x] User with fewer than `MAX_STRATEGIES_PER_USER` active strategies can create a new strategy successfully
- [x] User with exactly `MAX_STRATEGIES_PER_USER` active (non-archived) strategies gets HTTP 403 (or 429)
- [ ] Error response includes `code: "LIMIT_STRATEGIES_REACHED"`
- [x] Error response includes a friendly `message` (e.g., "You've reached the beta limit of 10 strategies.")
- [ ] Error response includes `limit` and `current` values
- [ ] Error response includes `reset_at: null` (not time-based)
- [x] Archived strategies do NOT count toward the max
- [x] After archiving a strategy, user can create a new one (slot is freed)
- [x] Duplicating a strategy also counts toward the limit and is rejected if at max

### 2.2 Count Accuracy

- [x] Count is derived from `strategies WHERE user_id = ? AND is_archived = false` (or equivalent)
- [x] Count is accurate immediately after archiving/unarchiving
- [x] Count is accurate immediately after creating/deleting strategies
...
- [x] User with fewer than `MAX_BACKTESTS_PER_USER_PER_DAY` runs today can create a new backtest
- [x] User at the daily limit gets HTTP 429
- [ ] Error response includes `code: "LIMIT_BACKTESTS_DAILY_REACHED"`
- [x] Error response includes a friendly `message` (e.g., "You've hit today's beta limit of 50 backtests. Try again tomorrow.")
- [ ] Error response includes `limit`, `current`, and `reset_at` (ISO timestamp for next rollover)
- [x] `reset_at` correctly reflects the next UTC midnight (or configured timezone rollover)

### 3.2 Counting Rules

- [x] All run records (pending, running, completed, failed) count toward the daily limit
- [ ] Retries for the same `run_id` do NOT increment the count (but a new run does)
- [x] Scheduled re-backtests (auto-triggered) count the same as manual runs
- [x] Failed runs count toward the limit (prevents spam)
...
- [x] Count is accurate immediately after creating a new run
...
- [x] When strategy creation is rejected due to limit, a clear toast/dialog is shown
- [x] Message includes actionable guidance: "Archive one to create another."
- [x] No server error (500) is shown; the error is handled gracefully

### 5.3 Limit Hit – Backtest Create

- [x] When backtest creation is rejected due to daily limit, a clear toast/dialog is shown
...
- [x] Page/section exists and is accessible
- [x] Content mentions OHLCV candle data (not tick/order book)
- [ ] Content mentions the fixed fee model (user default + optional override)
- [x] Content mentions simple slippage (extra cost per trade)
- [x] Content mentions MVP constraints: single asset, single timeframe
- [ ] Content includes the disclaimer: "Backtests are simulations and don't guarantee future results."
- [x] Content is concise (bullet list format, no walls of text)
- [x] Assumptions described match actual MVP behavior
...
- [x] Error messages are in plain language and actionable
...
- [x] Index exists on `backtest_runs(user_id, created_at)` or equivalent for fast daily counting (Option A)
...
- [x] User with exactly the limit number of strategies can still archive and re-create (slot freed)
...
- [x] Failed runs still count toward the daily limit
