...
> **Resolved (2026-06-06 grilling session) → ADR-0007.** Subscription is the primary engine (zero custody leaves no fat downstream basin to funnel into). It **gates on verification *depth* (`max_history_days`), never on idea *throughput*** — because the NL wedge auto-backtests every draft (ADR-0006), so gating `max_backtests_per_day` would throttle the magic moment. The wedge runs a cheap short-window backtest unthrottled; deep multi-cycle verification is the upsell; `max_backtests_per_day` is demoted to an anti-abuse ceiling. **Alerts** (the shipped signals-only surface — see decision #1) become an expansion lever *and* the §2 churn antidote. Cost is amortized by ADR-0002 (price cache) + ADR-0005 (version dedup). Consequence: the backtests/day gate must be reworked *before* the NL wedge ships.
...
1. **The shareable verified artifact (verification as content).** The most natural unit of distribution is *the honest result itself*: "I tested the 'buy every RSI<30' idea on ETH — here's what it actually did." A plain-English, screenshot-ready, slightly humbling result is *inherently shareable* in a way a Sharpe ratio is not. The narrative/"what you just learned" cards are latent distribution assets. Make every verified result a clean, public, linkable artifact and the product seeds its own top-of-funnel. This is the "Wordle result" pattern applied to trading beliefs.
   ...
   > **Correction (2026-06-06 grilling session):** this artifact already exists in code — the **Shared backtest** (`/share/backtests/{token}`, backed by `GET /backtests/share/{token}`). It is already token-gated and result-only (it explicitly hides the strategy graph: *"Strategy logic is not shared"*). So the work is **enrichment, not a spike**: the shared page shows metrics + equity curve but **not** the "what you just learned" narrative that is the actual distribution hook here. The job is to put the honest narrative onto the existing Shared backtest. See `CONTEXT.md` → **Shared backtest** for the canonical term and its distinction from **Publish**.
...
2. **"Build for agents" — Software 3.0 distribution.** Karpathy argues the new consumers of products are also LLMs. Two angles: (a) an `llms.txt`/clean docs surface so when someone asks an LLM "how do I backtest a crypto idea without code," Blockbuilders is legible and recommendable; (b) longer term, an MCP-style interface so a user's general AI assistant can *delegate* "test this idea" to Blockbuilders' trusted engine. Becoming **the verification backend that other AIs call** is a distribution moat the execution platforms aren't positioned for.
   ...
   > **Leverage note (2026-06-06 grilling session):** this is **the NL wedge with a different skin**. "Other AIs call the verification backend" is architecturally the same pipeline as decision #3 — NL → Strategy-validator gate → drafted graph → auto-backtest → **Shared backtest** link (ADR-0006). An MCP/HTTP "test this crypto idea" tool is a thin surface over machinery already being built, not a separate product — so among these three organic bets it has the highest leverage-per-effort. (Today it is entirely greenfield: no `llms.txt`, no tool surface; the only public endpoints are auth, health, and `/backtests/share/{token}`.)
...
> **Reframed (2026-06-06 grilling session): the axis is compounding-vs-rented, not organic-vs-paid.**
> - **LTV premise updated.** "Execution-free ⟹ low LTV" is no longer structural — ADR-0007 (depth-gated subscription) plus the **Alert** churn-antidote (ping dormant users between §2 pain-spikes) make LTV a *designed* variable. The honest claim is narrower: even an optimized LTV probably can't out-bid execution-LTV incumbents in a performance-marketing auction. Conclusion survives; reasoning must reference ADR-0007, not a fixed "low LTV."
> - **Don't commit to organic-*only*.** "The product seeds its own funnel" is a hope with a named falsifier (§9: "nobody shares"). Prohibiting paid entirely means that if the Shared backtest doesn't go viral, you've banned the one lever that reliably moves volume — that is ideology, not strategy.
> - **The real rule:** spend (paid or not) only on **compounding, brand-consistent assets** — education/SEO content, the agent-legibility surface, shareable results — which keep paying after the spend stops. Refuse specifically the **rented, brand-eroding CAC auction** against incumbents (stops when you stop paying; "we paid to put you here" clashes with the honesty brand). The trap is the auction, not "paid" categorically.
...
- **Guardrail:** signals are still verification-gated. We only let a user wire up an alert for a strategy they've *backtested* — the signal inherits the trust of the engine. No "blind" alerts. The moment a signal feels like a tip rather than a verified trigger, the trust moat erodes.
  - *Sharpened (2026-06-06 grilling session):* verification-gating is a **binding, not a one-time check**. A performance **Alert** must bind to the specific **frozen Strategy version** that was backtested — *not* the mutable working copy (ADR-0005). Otherwise a user edits the working copy after wiring up (e.g. `RSI<30` → `RSI<50`, no re-backtest) and gets pinged on logic that was never verified — precisely the "un-verified tip" residual risk below. Editing the working copy must not change what a live Alert watches; re-verifying yields a new version the user deliberately re-binds. See `CONTEXT.md` → **Alert**.
  - *Reality check (same session):* **this is not a future feature — the `Alert` system already ships** (`alert_rules`, `AlertsApiClient`, `alert_evaluator`), and it currently *violates* this guardrail. **Performance alerts** bind to `strategy_id` (not a version) and are not verification-gated at creation — the working-copy-drift leak above is live. **Price alerts** are bare asset+threshold tripwires evaluated on **spot price** with optional **webhook export** to execution platforms — no strategy, no backtest: exactly the "blind alert" this guardrail forbids. So decision #1's real work is **hardening the shipped Alert system to these rules**, not building a later "signals-only handoff." The blind price→webhook path needs a deliberate product/trust decision (gate it behind a backtested strategy, or split it off as an explicitly un-verified "price tripwire" and stop branding the whole surface verification-gated). *Flagged for a security/trust review.*
...
4. **Enrich the existing Shared backtest** (§7) — the public, linkable, result-only artifact (`/share/backtests/{token}`) already ships. This is *not* a spike: add the plain-English "what you just learned" narrative to it (the actual distribution hook), and make it screenshot-clean. Keep it result-only and verification-gated (see `CONTEXT.md` → **Shared backtest**).
5. **Harden the *already-shipped* Alert system** (decision #1) — this is not a greenfield "later" build; `alert_rules` / `AlertsApiClient` / `alert_evaluator` are live. The work: (a) version-bind + verification-gate performance alerts (fire on the frozen backtested version, on candle close — see decision #1 guardrail); (b) make a deliberate call on the blind price→webhook tripwire (gate it or split it off as explicitly un-verified); (c) instrument Alert creation/activation as the **executor-intent signal** for the §9 iterators-vs-executors split — the measurement instrument already exists, so the split is readable now (cleanest once (a)/(b) land). Sequence behind activation + wedge proof for *new* investment, but the hardening/gating is a correctness debt on a shipped surface, not a future feature.
