...
- **Guardrail:** signals are still verification-gated. We only let a user wire up an alert for a strategy they've *backtested* — the signal inherits the trust of the engine. No "blind" alerts. The moment a signal feels like a tip rather than a verified trigger, the trust moat erodes.
  - *Sharpened (2026-06-06 grilling session):* verification-gating is a **binding, not a one-time check**. A Trigger must bind to the specific **frozen Strategy version** that was backtested — *not* the mutable working copy (ADR-0005). Otherwise a user edits the working copy after wiring up (e.g. `RSI<30` → `RSI<50`, no re-backtest) and gets pinged on logic that was never verified — precisely the "un-verified tip" residual risk below. Editing the working copy must not change what a live Trigger watches; re-verifying yields a new version the user deliberately re-binds. See `CONTEXT.md` → **Trigger**.
