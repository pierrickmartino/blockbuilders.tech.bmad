...
- **⚠️ Resolved design — see ADR-0021 for the full design.** This 2026-06-05 framing reads as greenfield, but the Alert system **already ships** (`alert_rules`, both performance + price subtypes, evaluation, notification, scheduler — ADR-0007: "harden + gate, not build"). #16 is *that* hardening of the **performance alert** only (the **price alert** is the un-verified "blind alert", walled off and untouched): make it **result-anchored** and **pinned to the frozen `strategy_version` whose verdict was viewed** (verification-gating falls out of the surface), fire via a **timeframe-aligned full re-backtest of the pinned version on closed candles only** (not the daily `auto` cron, not the forming candle), **alert-driven** (`triggered_by='alert'`, decoupled from `auto_update_enabled`) with a `last_fired_candle_ts` watermark and coalesced catch-up. One active alert per strategy; re-pin is deliberate; passive edits never move the pin (surfaced as a **stale alert**); archive deactivates, delete cascades. Existing rows (2–5 users) are **deactivated + notified**, not backfilled. The Premium entitlement gate (ADR-0007) is a noted follow-up, out of this slice.
...
- **⚠️ Resolved design — see ADR-0022 for the full design.** This 2026-06-05 title predates ADR-0021 and bundles three architecturally unrelated mechanisms under one name. Grilled down, #17 is *one* thing: a trust-inheriting **outbound webhook on the #16/ADR-0021 performance alert** (reusing the existing `alert_rules.notify_webhook`/`webhook_url` columns — no new table), so it **depends on #16**. Four things the title gets wrong: (1) it is a live **webhook channel**, not a downloadable artifact; (2) **TradingView is dropped** as a destination — it is a webhook *source*, not a sink you can POST to — leaving a **generic outbound JSON POST** to any inbound-webhook sink (3Commas signal bots, Cornix, relays); (3) the payload is a **signal, never an order** — event + asset + timeframe + closed `candle_ts` + strategy name + result link, deliberately omitting side/size/leverage/price-target and the graph, so "the hand-off, never the trade" stays literally true; (4) delivery is **one POST per discrete event** (not coalesced like the human-facing Notification), fire-and-forget, `follow_redirects=False`, watermark advancing regardless of delivery. **Rejected (not deferred):** TradingView **Pine generation** (a lossy block-graph→Pine port forks the engine's verdict — anti-thesis to the ADR-0017 trust moat) and a static **3Commas config** (nothing meaningful to emit — signal bots are driven by the very webhook this builds). **Split off:** **CSV** → its own data-export slice (result data, unrelated to execution). **Ungated for now:** the ADR-0007 Premium lever folds into the ADR-0021 entitlement follow-up. **Follow-up:** outbound-webhook egress hardening (DNS-rebinding/TOCTOU). Config lives in `PerformanceAlertPanel` with an inline honesty disclosure. See also CONTEXT.md → **Execution handoff**.
...
- **⚠️ Resolved design — see ADR-0024 for the full design.** This 2026-06-05 framing reads as a greenfield pair ("comparison plus coaching"), but the **side-by-side comparison already ships** end-to-end (`backend/app/api/backtest_compare.py`, `frontend/src/app/(app)/strategies/[id]/backtest/compare/page.tsx`, `BacktestRunsList`, `AllRunsDrawer`). #19 collapses to the missing *interpretation* layer — **Tweak coaching** (CONTEXT.md): a deterministic, comparative, diff-attributed explanation of *why* a re-tested **Strategy version** differs from an earlier one, on an **"Explain this delta"** action on that compare surface. Five things the title's "why your idea underperformed" gets wrong or imprecise: (1) it is **comparative** (version-vs-version), not single-run; (2) when the two runs' windows differ it re-runs both versions over the **intersection** through the real engine (a **Comparison run**, `triggered_by='comparison'`, walled off from history / **Activation** / alert-pinning) rather than slicing stored results — every number stays engine-truth; (3) causation is asserted **only** when *every* edit is a risk-block param change (a **Strategy diff** gate: signals byte-identical, trades matched by `entry_time`, each exit self-attributed via `exit_reason`) — any structural change drops to a **descriptive, no-cause** tier; (4) it **never speculates** ("would have recovered") — it reports the counterpart run's *observed* outcome ("the same entry closed at +4% under your previous stop"); (5) per ADR-0004's single-position model, "same entries" holds only up to the first divergence, so matching is a three-bucket set-diff, not a 1:1 zip. The §6/2026-06-15 ADR-0010 gate is **re-scoped** (ADR-0010 amendment, 2026-06-18): **build now, ship prominently on a non-negative #3**. See also CONTEXT.md → **Tweak coaching**, **Strategy diff**, **Comparison run**.
